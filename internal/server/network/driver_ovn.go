...
// checkFamilyListenAddress returns an error if another network on our logical router uses the listen address.
func (n *ovn) checkFamilyListenAddress(listenAddress string) error {
	family, err := n.routerFamily()
	if err != nil {
		return err
	}
...
// routerFamily returns the networks sharing our logical router, starting with its owner.
func (n *ovn) routerFamily() ([]*ovn, error) {
	owner := n
	...
	family := append([]*ovn{owner}, children...)

	// We aren't part of the created networks yet while our own creation is in progress.
	if !slices.ContainsFunc(family, func(member *ovn) bool { return member.ID() == n.ID() }) {
...
func (n *ovn) populateAutoConfig(config map[string]string) error {
	...
	var familySubnets []*net.IPNet

	if n.state != nil && config["parent"] != "" && (config["ipv4.address"] == "auto" || config["ipv6.address"] == "auto") {
		var err error

		familySubnets, err = n.routerFamilySubnets(config["parent"])
		if err != nil {
			return err
		}
	}
	...
	if config["ipv4.address"] == "auto" {
		subnet, err := generateUnusedSubnet(randomSubnetV4, familySubnets)
		if err != nil {
			return err
		}
	...
	if config["ipv6.address"] == "auto" {
		subnet, err := generateUnusedSubnet(randomSubnetV6, familySubnets)
		if err != nil {
			return err
		}
...
func (n *ovn) validateParentNetwork(config map[string]string) error {
	...
	for _, key := range []string{"ipv4.address", "ipv6.address"} {
		...
		if subnetOverlapsAny(subnet, familySubnets) {
...
// routerFamilySubnets returns the internal subnets of the other networks on the parent's logical router.
func (n *ovn) routerFamilySubnets(parentName string) ([]*net.IPNet, error) {
...
func (n *ovn) logicalRouterPolicySetup(ovnnb *networkOVN.NB, excludePeers ...int64) error {
	...
	err = family[0].forPeers(func(targetOVNNet *ovn) error {
		...
		comment := family[0].getLogicalRouterPeerPortName(targetOVNNet.ID())
...
func (n *ovn) deleteRouterNetworkConfig() error {
	...
	familySubnets, err := n.routerFamilySubnets(n.config["parent"])
	if err != nil {
		return err
	}
	...
	for _, subnet := range []*net.IPNet{routerIntPortIPv4Net, routerIntPortIPv6Net} {
		if subnet != nil && !subnetOverlapsAny(subnet, familySubnets) {
...
func (n *ovn) ForwardCreate(forward api.NetworkForwardsPost, clientType request.ClientType) error {
	...
	if clientType == request.ClientTypeNormal {
		...
		err = n.checkFamilyListenAddress(forward.ListenAddress)
		if err != nil {
			return err
		}
...
func (n *ovn) LoadBalancerCreate(loadBalancer api.NetworkLoadBalancersPost, clientType request.ClientType) error {
	...
	if clientType == request.ClientTypeNormal {
		...
		err = n.checkFamilyListenAddress(loadBalancer.ListenAddress)
		if err != nil {
			return err
		}
