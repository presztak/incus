...
func (d *nicBridged) validateConfig(instConf instance.ConfigReader, partialValidation bool) error {
	...
	optionalFields := []string{
		...
		// gendoc:generate(entity=devices, group=nic_bridged, key=limits.burst.length)
		//
		// ---
		//  type: string
		//  default: `1s`
...
func (d *nicBridged) UpdatableFields(oldDevice Type) []string {
	...
	return []string{"limits.ingress", "limits.egress", "limits.max", "limits.ingress.burst", "limits.egress.burst", "limits.max.burst", "limits.burst.length", "limits.priority", "ipv4.routes", "ipv6.routes", "ipv4.routes.external", "ipv6.routes.external", "ipv4.address", "ipv6.address", "security.mac_filtering", "security.ipv4_filtering", "security.ipv6_filtering", "security.acls", "security.acls.default.egress.action", "security.acls.default.egress.logged", "security.acls.default.ingress.action", "security.acls.default.ingress.logged", "connected"}
}
...
func allowedIPNets(config deviceConfig.Device) (IPv4Nets []*net.IPNet, IPv6Nets []*net.IPNet, err error) {
	getAllowedNets := func(ipVersion int) ([]*net.IPNet, error) {
		if util.IsFalseOrEmpty(config[fmt.Sprintf("security.ipv%d_filtering", ipVersion)]) {
...
func (d *nicBridged) State() (*api.InstanceStateNetwork, error) {
	...
	if d.network != nil {
		...
		if d.config["hwaddr"] != "" {
			...
			if util.IsFalseOrEmpty(d.network.Config()["ipv6.dhcp.stateful"]) && v6subnet != nil {
