...
func (d *nicP2P) validateConfig(instConf instance.ConfigReader, partialValidation bool) error {
	...
	optionalFields := []string{
		...
		// gendoc:generate(entity=devices, group=nic_p2p, key=limits.burst.length)
		//
		// ---
		//  type: string
		//  default: `1s`
...
func (d *nicP2P) UpdatableFields(oldDevice Type) []string {
	...
	return []string{"limits.ingress", "limits.egress", "limits.max", "limits.ingress.burst", "limits.egress.burst", "limits.max.burst", "limits.burst.length", "limits.priority", "ipv4.routes", "ipv6.routes", "connected"}
}
